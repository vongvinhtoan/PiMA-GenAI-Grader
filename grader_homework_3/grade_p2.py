...
def plot_function_conture():
    ...
    cbar.set_label('Giá trị của hàm $f(\mathbf x)$')
    plt.title("Biểu đồ nhiệt hàm $f(\mathbf x)$")
    plt.xlabel("$x$")
    plt.ylabel("$y$")
    plt.grid(True)
    plt.axis("equal")
...
def plot_function_derivative_vector_field():
    ...
    ax2.set_title("Trường vector của $\\nabla f(\mathbf x)$ + Đường đồng mức của $f(\mathbf x)$")
    ax2.set_xlabel("$x$")
    ax2.set_ylabel("$y$")
    ax2.axis("equal")
    ax2.grid(True)
...
def test_function_contour(f_tested):
    ...
    cbar1.set_label('Giá trị của hàm $f_{tested}(\mathbf x)$')
    ax1.set_title("Biểu đồ nhiệt hàm $f_{tested}(\mathbf x)$")
    ax1.set_xlabel("$x$")
    ax1.set_ylabel("$y$")
    ax1.grid(True)
    ax1.axis("equal")
    ...
    cbar2.set_label('Giá trị của hàm $f_{jury}(\mathbf x)$')
    ax2.set_title("Biểu đồ nhiệt hàm $f_{jury}(\mathbf x)$")
    ax2.set_xlabel("$x$")
    ax2.set_ylabel("$y$")
    ax2.grid(True)
    ax2.axis("equal")
...
def test_function_derivative_vector_field(grad_f):
    ...
    ax2.set_title("Trường vector của $\\nabla f(\mathbf x)$ với overlay kết quả")
    ax2.set_xlabel("$x$")
    ax2.set_ylabel("$y$")
    ax2.axis("equal")
    ax2.grid(True)
